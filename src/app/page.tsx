...
export default function KoreanCrisisHomePage() {
  const [articles, setArticles] = useState<NewsArticle[]>([]);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [selectedCategory, setSelectedCategory] = useState<Category>("전체 상황실");
  ...
  const fetchArticles = useCallback(async () => {
    try {
      ...
      if (selectedCategory !== "전체 상황실") params.append("category", selectedCategory);
    ...
    <div className="min-h-screen flex flex-col justify-between">
      ...
      <div>
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
          ...
          {selectedCategory === "전체 상황실" && !searchQuery.trim() && articles.length > 0 && (
            <HeroFeatured
              articles={articles}
              locale="ko"
      ...
      <footer className="mt-16 border-t border-slate-200/80 dark:border-slate-800 bg-white/70 dark:bg-slate-950/70 py-8 px-4 text-center text-xs text-slate-500 dark:text-slate-400">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-slate-800 dark:text-slate-200">글로벌 크라이시스 & AI 펄스 (CrisisPulse)</span>
            <span>• 전 지구 AI 안보, 기후재난 & 인도주의 구호 네트워크</span>
          </div>
          <p>© 2026 CrisisPulse. Dedicated to Global Peace, Climate Resilience & Ethical AI.</p>
        </div>
      </footer>
