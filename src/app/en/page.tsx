...
export default function EnglishCrisisHomePage() {
  const [articles, setArticles] = useState<NewsArticle[]>([]);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [selectedCategory, setSelectedCategory] = useState<Category>("All Situation Room");
  ...
  const fetchArticles = useCallback(async () => {
    try {
      ...
      if (selectedCategory !== "All Situation Room") params.append("category", selectedCategory);
  ...
  const categoryCounts = articles.reduce((acc, a) => {
    ...
    acc["All Situation Room"] = (acc["All Situation Room"] || 0) + 1;
    return acc;
  ...
  return (
    <div className="min-h-screen flex flex-col justify-between">
      ...
      <div>
        {/* Planetary Threat Marquee Ticker */}
        <CrisisTicker locale="en" />
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
          ...
          {/* Hero Featured Lead Alert */}
          {selectedCategory === "All Situation Room" && !searchQuery.trim() && articles.length > 0 && (
            <HeroFeatured
              articles={articles}
              locale="en"
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 my-6 pt-4 border-t border-slate-200/80 dark:border-slate-800">
            <div className="flex items-center gap-2">
              <span className="p-2 rounded-xl bg-cyan-100 dark:bg-cyan-950/50 text-cyan-700 dark:text-cyan-400">
                <ShieldAlert className="w-4 h-4" />
              </span>
              <h2 className="text-base sm:text-lg font-black text-slate-900 dark:text-white tracking-tight">
                {selectedCategory} Real-Time Situation Reports
              </h2>
              <span className="px-2.5 py-0.5 rounded-full text-xs font-mono font-bold bg-cyan-50 text-cyan-700 dark:bg-cyan-950/60 dark:text-cyan-400 border border-cyan-200 dark:border-cyan-800">
          ...
          {loading ? (
            ...
            <div className="text-center py-20 rounded-3xl bg-white dark:bg-slate-900 border border-dashed border-slate-200 dark:border-slate-800">
              <Sparkles className="w-10 h-10 mx-auto text-cyan-400 mb-3 animate-pulse" />
              <h3 className="text-base font-bold text-slate-800 dark:text-slate-200 mb-1">
                No situation reports match your search criteria
              </h3>
              <p className="text-xs text-slate-400">
                Try searching for Ukraine, Wildfires, UN Peace, or Satellite AI.
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
      ...
      <footer className="mt-16 border-t border-slate-200/80 dark:border-slate-800 bg-white/70 dark:bg-slate-950/70 py-8 px-4 text-center text-xs text-slate-500 dark:text-slate-400">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-slate-800 dark:text-slate-200">CrisisPulse</span>
            <span>• Global Conflict, Climate Disasters & AI Planetary Solutions Platform</span>
          </div>
          <p>© 2026 CrisisPulse. Dedicated to Global Peace, Planetary Resilience & Ethical AI.</p>
        </div>
      </footer>
